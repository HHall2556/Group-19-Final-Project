...
nodf = df[
    ['Address', 'State', 'County', 'City', 'Date Local', 'O3 Units', 'O3 Mean', 'O3 1st Max Value', 'O3 1st Max '
                                                                                                       'Hour',
     'O3 AQI']]
...
for i in fullyeardf.index:
    mapA.append(str(fullyeardf['O3 Mean'].iloc[i])[:5] + ' ppm')
...
for i in winterdf.index:
    mapA.append(str(winterdf['O3 Mean'].iloc[i])[:5] + ' ppm')
...
for i in springdf.index:
    mapA.append(str(springdf['O3 Mean'].iloc[i])[:5] + ' ppm')
...
for i in summerdf.index:
    mapA.append(str(summerdf['O3 Mean'].iloc[i])[:5] + ' ppm')
...
for i in autumndf.index:
    mapA.append(str(autumndf['O3 Mean'].iloc[i])[:5] + ' ppm')
...
data = dict(type='choropleth',
            ...
            z=fullyeardf['O3 Mean'],
            locationmode='USA-states',
            text=fullyeardf['text'],
            marker=dict(line=dict(color='rgb(255,255,255)', width=2)),
            colorbar={'title': "O3 Mean in ppm"}
            )
layout = dict(title='O3 Mean Value in 2015 by State',
              geo=dict(scope='usa',
                       showlakes=True,
                       lakecolor='rgb(85,173,240)')
              )
choromap = go.Figure(data=[data], layout=layout)
pyo.plot(choromap, filename='O3_Map.html')
