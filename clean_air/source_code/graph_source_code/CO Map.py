...
nodf = df[
    ['Address', 'State', 'County', 'City', 'Date Local', 'CO Units', 'CO Mean', 'CO 1st Max Value', 'CO 1st Max '
                                                                                                       'Hour',
     'CO AQI']]
...
for i in fullyeardf.index:
    mapA.append(str(fullyeardf['CO Mean'].iloc[i])[:5] + ' ppm')
...
data = dict(type='choropleth',
            ...
            z=fullyeardf['CO Mean'],
            locationmode='USA-states',
            text=fullyeardf['text'],
            marker=dict(line=dict(color='rgb(255,255,255)', width=2)),
            colorbar={'title': "CO Mean in ppm"}
            )
layout = dict(title='CO Mean Value in 2015 by State',
              geo=dict(scope='usa',
                       showlakes=True,
                       lakecolor='rgb(85,173,240)')
              )
choromap = go.Figure(data=[data], layout=layout)
pyo.plot(choromap, filename='CO_Map.html')